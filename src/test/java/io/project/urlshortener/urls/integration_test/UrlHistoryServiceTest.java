...
public class UrlHistoryServiceTest {
	...
	void test(){
		...
		UrlHistoryDto urlHistory1st = urlHistoryService.generateShortUrl(userInputUrl);
		...
		UrlHistoryDto urlHistory2nd = urlHistoryService.generateShortUrl(userInputUrl);
		System.out.println(urlHistory2nd);

		Assertions.assertThat(urlHistory2nd.getRequestCnt()).isEqualTo(2);

		// 3 번째 호출은 축약 URL로 시도해본다
		UrlHistoryDto urlHistory3nd = urlHistoryService.generateShortUrl(shortUrl);
